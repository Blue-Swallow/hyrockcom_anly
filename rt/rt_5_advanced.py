# -*- coding: utf-8 -*-
"""
Created on Sat Nov 11 00:36:51 2020
...
class Main:
    def __init__(self, ex_df, func_cstr, func_gamma, input_param, plot=False):
        ...
        self.counter_of_improv = 0
        self.rt5 = rt_5.Main(ex_df, func_cstr, func_gamma, input_param, plot=False)
        
    def execute_RT(self, maxiter=30, eta_init=0.8, max_cycle=10):
        for counter in range(max_cycle+1):
            if counter == 0:
                print("\nExecute RT-5 calculation")
            else:
                if counter == 1:
                    string = "st"
                elif counter == 2:
                    ...
                    string = "th"
                print("\n{}".format(counter)+ string + " O/F improvemnt cycle")
            try:
                eta_init = self.rt5.iterat_newton_eta(maxiter, eta_init=eta_init)
            except:
                eta_init = self.rt5.iterat_brentq_eta(maxiter, eta_min=0.1, eta_max=2.0)
            if False:
                print("Stop the O/F improvement cycle because multiple solution was observed.")
                print("xxx value = aaaa.\n")
                break
            else:
                print("Multiple solution was not occureds.")
                print("xxx value = aaaa\n")
                self.counter_of_improv = counter
                self.rt5.of_rt1 = self.rt5.anl_df.of    # To improve O/F, O/F calculated by rt5 subject to the next rt5 calculation
                tmp_anl_df = self.rt5.anl_df.copy()
        self.anl_df = tmp_anl_df
